...
public class Settings implements Screen {
	...
	public static FileHandle levelDirectory() {
		String prefsDir = Gdx.app.getPreferences(KanjiKadoGame.TITLE).getString("leveldirectory").trim();
		if(prefsDir != null && !prefsDir.equals(""))
			return Gdx.files.absolute(prefsDir);
		else
			return Gdx.files.absolute(Gdx.files.external(KanjiKadoGame.TITLE + "/levels").path()); // return default level directory
	}

	/** @return if vSync is enabled */
	public static boolean vSync() {
		return Gdx.app.getPreferences(KanjiKadoGame.TITLE).getBoolean("vsync");
	}
		...
		ClickListener buttonHandler = new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				...
				if(event.getListenerActor() == vSyncCheckBox) {
					// save vSync
					Gdx.app.getPreferences(KanjiKadoGame.TITLE).putBoolean("vsync", vSyncCheckBox.isChecked());

					// set vSync
					Gdx.graphics.setVSync(vSync());

					Gdx.app.log(KanjiKadoGame.TITLE, "vSync " + (vSync() ? "enabled" : "disabled"));
				} else if(event.getListenerActor() == back) {
					// save level directory
					String actualLevelDirectory = levelDirectoryInput.getText().trim().equals("") ? Gdx.files.getExternalStoragePath() + KanjiKadoGame.TITLE + "/levels" : levelDirectoryInput.getText().trim(); // shortened form of an if-statement: [boolean] ? [if true] : [else] // String#trim() removes spaces on both sides of the string
					Gdx.app.getPreferences(KanjiKadoGame.TITLE).putString("leveldirectory", actualLevelDirectory);

					// save the settings to preferences file (Preferences#flush() writes the preferences in memory to the file)
					Gdx.app.getPreferences(KanjiKadoGame.TITLE).flush();

					Gdx.app.log(KanjiKadoGame.TITLE, "settings saved");
